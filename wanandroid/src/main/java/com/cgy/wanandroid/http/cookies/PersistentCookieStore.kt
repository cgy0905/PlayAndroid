package com.cgy.wanandroid.http.cookies
...
class PersistentCookieStore {
    ...
    init {
        ...
        for (entry in prefsMap) {
            ...
            for (name in cookieNames) {
                val encodeCookie = cookiePrefs.getString(name, null)
                if (encodeCookie != null) {
                    val decodedCookie = decodeCookie(encodeCookie)
    ...
    fun add(url: HttpUrl, cookie: Cookie) {
        ...
        prefsWriter.putString(name, encodeCookie(
            OkHttpCookies(
                cookie
            )
        ))
    ...
    fun getCookies(): List<Cookie> {
        val ret = java.util.ArrayList<Cookie>()
        for (key in cookies.keys) {
