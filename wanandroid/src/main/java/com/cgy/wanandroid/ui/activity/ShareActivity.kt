...
import com.cgy.wanandroid.adapter.ShareAdapter
...
import com.cgy.wanandroid.mvp.presenter.SharePresenter
...
class ShareActivity : BaseMvpSwipeBackActivity<ShareContract.View, SharePresenter>(),
    ...
    override fun showShareList(data: ShareResponseBody) {
        data.shareArticles.datas.let {
            shareAdapter.run {
                if (isRefresh) {
                    replaceData(it)
                } else {
                ...
                pageSize = data.shareArticles.size
                if (data.shareArticles.over) {
